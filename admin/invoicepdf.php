...
if(isset($_GET["purchaseNo"]) && isset($_GET["custno"])) {
    ...
    if($result_customer && $result_order && mysqli_num_rows($result_customer) > 0 && mysqli_num_rows($result_order) > 0) {
        ...
        // Display total
        $htmlContent .= '<p style="text-align: right;"><b>รวมทั้งสิ้น:</b> ' . number_format($total, 2) . ' บาท</p><br></br>';
        
        $newDate = date('Y-m-d', strtotime($date . ' +3 days'));
        $htmlContent .= '<p>วันครบกำหนดชำระเงิน <span style="color: red;"> '.$newDate.'</span></p>';
        $htmlContent .= '<p>สถานะ '.$status.'</p>';
