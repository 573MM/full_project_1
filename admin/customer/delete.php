<?php
    $id =  $_GET["data"];
    $query = "SELECT * FROM customer WHERE CustNo = '$id'";
    $cx = mysqli_connect("localhost","root","","app");
    $result = mysqli_query($cx, $query);
    $row = mysqli_fetch_assoc($result);
    echo "ต้องการลบ รหัส : ". $row['CustNo'] ."<br>ชื่อ : ". $row['CustName'] ."<br> เพศ : ". $row['Sex']."<br>ที่อยู่ : ". $row['Address']."<br>เบอร์โทรศัพท์ : ". $row['Tel'];
    mysqli_close($cx);
?>
<form method="get" action="delete_form.php">
    <input type="hidden" name="data" value="<?php echo $row['CustNo']; ?>">
