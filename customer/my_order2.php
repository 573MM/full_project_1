...
<body>
    ...
    $query = "SELECT `orders`.PurchaseNo AS OrderNo, `orders`.ProductNo AS ProductNo, product.ProductName AS ProductName, product.PricePerUnit AS PricePerUnit, `orders`.ProductQty AS ProductQty
    FROM `orders`
    INNER JOIN product ON `orders`.ProductNo = product.ProductNo
    WHERE `orders`.CustNo = '$custId'
    ORDER BY `orders`.PurchaseNo"; 
